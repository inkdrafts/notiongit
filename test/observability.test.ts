...
const EVENTS: ProvisioningEvent[] = [
  { type: 'consent_started', jobId: 'job-1', ts: 1_000, provider: 'notion' },
  { type: 'consent_completed', jobId: 'job-1', ts: 1_100, provider: 'notion', templateDuplicated: true },
  { type: 'consent_failed', jobId: 'job-1', ts: 1_200, provider: 'notion', errorCode: 'notion_rate_limited' },
  { type: 'job_queued', jobId: 'job-1', ts: 1_300 },
  { type: 'job_enqueue_failed', jobId: 'job-1', ts: 1_400, errorCode: 'provisioning_enqueue_failed' },
  { type: 'step_started', jobId: 'job-1', ts: 1_500, step: 'configure_pages', attempt: 2 },
  { type: 'step_succeeded', jobId: 'job-1', ts: 1_600, step: 'configure_pages', attempt: 2, durationMs: 350 },
  {
    type: 'step_failed',
    jobId: 'job-1',
    ts: 1_700,
  ...
  { type: 'job_succeeded', jobId: 'job-1', ts: 1_900, totalDurationMs: 120_000 },
  {
  ...
  { type: 'status_rerun_dispatched', requestLabel: 'request-label-1', ts: 2_100 },
];
...
async function funnelEventsForCaughtError(
  error: unknown,
  attempts: number,
): Promise<{ logs: string[]; job: ProvisioningJob | null }> {
...
describe('redaction canary', () => {
  test('a provider error whose message carries a token never reaches the log sink or the job record', async () => {
    ...
    const { logs, job } = await funnelEventsForCaughtError(rateLimited, 0);
  ...
  test('an unclassified error whose message carries a token dead-letters without leaking it', async () => {
    ...
    const { logs, job } = await funnelEventsForCaughtError(raw, PROVISIONING_STEP_MAX_ATTEMPTS - 1);
