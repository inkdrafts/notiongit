...
async function finishGithubCallback(request: Request, env: Partial<Env>): Promise<Response> {
  ...
  try {
    ...
    if (code) {
      ...
      // L5: the lease belongs to this request until a job is durably created
      // and enqueued; if anything below fails, release it before returning,
      // so a crashed request never locks the account out for a full TTL.
